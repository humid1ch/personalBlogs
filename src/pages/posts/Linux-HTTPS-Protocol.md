...
alt: 'cover'
tags: ["Linux网络", "应用层", "协议", "HTTPS", "约4443字 -- 阅读时间≈18分钟"]
theme: 'light'
featured: false
---
...
1. `HTTP`协议以明文的形式传输数据, 缺乏对信息的保护. 
    ...
    想象一下, 如果你在和喜欢的人在网上表白的时候, 有人把你发送的消息在网络中改了… 😱😱😱
...
> 关于安全的理解. 
>
> 一般认为, 只要获取数据的成本 远比 获取到数据后可获取的利益大, 就说是安全的.
>
> 当然并不绝对
...
但是, 中间人获取到公钥之后, **可以解密 服务器使用私钥加密的数据**.
...
答案是有的. 那就是 先非对称加密协商公钥, 再通过公钥加密 对称密钥 将加密的密钥发送给私钥持有者.
...
那么, 如果 **让客户端可以识别包含公钥的数据是否直接来自服务器** 就可以解决这个麻烦. 
