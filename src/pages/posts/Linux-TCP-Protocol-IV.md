...
title: '[Linux] TCP协议介绍(4): 滑动窗口、快重传机制、流量控制、拥塞控制、粘包等概念 简单介绍分析...'
pubDate: 2024-01-20
...
`TCP`协议是面向字节流的, 面向字节流就表示在数据传输时, 应用层发送的数据在传输层不会存在数据边界

即, 如果应用层发送`1 + 1`和`2 + 2`, `TCP`协议不会在乎什么`1 + 1`和`2 + 2`, 而是很可能因为要发送的数据内容太小 而将`1 + 1`和`2 + 2`按顺序 **无间隔** 的放在`TCP`的发送缓冲区中, 等待将数据一起发送走

那么, 此时`TCP`就有可能将数据以`1 + 12 + 2`的形式发走, 而 **类似这种现象就被称为`TCP`粘包**

此时, 如果接收方读取数据, 很可能一次性读取到`1 + 12 + 2`, 从而获得非期望的结果, **这是粘包可能造成的影响**

不仅是 发送方因考虑发送数据效率 而 **将多条数据粘在一起进行封包发送**, 是粘包

并且, 如果发送方确实做到了每条数据都单独封包发送, 但是接收方应用层没有及时将数据从接收缓冲区中读取走, 而是将 **发送方单独发送的多条数据 堆积在了接收方的`TCP`接收缓冲区中**, 这种情况 也是发生了粘包

也就是说, **因为`TCP`是面向字节流的, 数据会以流的形式存储在发送、接收缓冲区中, 那么在`TCP`通信中, 就有可能发生这种: 应用层发送的多条数据 堆积在发送、接收缓冲区中, 导致多条数据没有间隔的粘在一起的现象, 就是粘包**

粘包是在`TCP`协议缓冲区中会发生的现象, **如果应用层没有对数据进行额外的边界限制, 就会造成应用层无法分析每条数据的边界, 进而导致应用层无法正确处理数据**

当然, 也可以将粘包理解为, 多条数据在`TCP`缓冲区中没有边界的连续存储在一起, **如果每条数据本身存在边界, 也可以不认为发生粘包**

## 如何避免 或 减少粘包

已经了解了什么是粘包, 那么粘包就可以从几个方面来避免或减少:

1. 在对端窗口允许的情况下, 发送端将应用层发送的每条数据都单独发送走, 这样从发送方减少了粘包的发生的可能
2. 在接收端, 应用层要及时处理 传输层接收缓冲区的数据, 尽量避免数据在接收缓冲区堆积
