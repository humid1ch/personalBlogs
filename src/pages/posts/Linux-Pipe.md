...
`Linux`系统中 描述进程已打开文件 的结构体`files_struct`, 其中存储着 指向打开文件的数组`fd_array`, 此数组的类型是 `struct files*`. 
...
这是管道的特点, `Linux`的管道也是遵循这个特点的, 也就是说: 
...
2. 系统调用创建
	...
	`Linux`除了给了`mkfifo`命令, 还给了`mkfifo()`系统调用:
